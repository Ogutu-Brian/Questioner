from . import (user_data, meetup_data, QuestionData,
               question_data, user_data, meetup_data, db, BaseTest)
...
class TestQuestion(BaseTest):
    ...
    def test_successful_upvote(self):
        ...
        result = json.loads(self.client().patch(url).get_data(as_text=True))
        self.assertEqual(status.created, result.get("status"))
